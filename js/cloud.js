...
window.Cloud = (function(){
  ...
  async function push(){
    ...
    } catch(e){ setStatus("Sauvegarde en ligne impossible (hors-ligne ?)."); }
  }
  ...
  async function pullAndMerge(){
    ...
    } catch(e){ setStatus("Lecture en ligne impossible."); }
  }
  ...
  function render(){
    ...
    if (!box) return;
    if (!isConfigured()) {
      box.innerHTML = '<h3>' + Icon("cloud") + 'Mon compte</h3><p class="acct-note">Les comptes ne sont pas encore activés dans cette version. Ta progression est sauvegardée sur cet appareil.</p>';
      return;
    }
    if (user) {
      box.innerHTML =
        ...
        '<div class="acct-btns">' +
        '<button class="btn small" id="acct-sync">' + Icon("refresh") + 'Synchroniser maintenant</button>' +
